...
Add to `~/.tmux.conf` (replace `<your-fork-or-remote>` with wherever this repo actually lives — TPM just needs a git URL or `user/repo` shorthand for a GitHub-hosted one):

```tmux
set -g @plugin '<your-fork-or-remote>/tmux-ui-manager'
```
...
```sh
git clone <your-fork-or-remote>/tmux-ui-manager ~/.tmux/plugins/tmux-ui-manager
