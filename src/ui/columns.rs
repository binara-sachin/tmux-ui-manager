...
fn render_sessions(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) {
    ...
    let mut items: Vec<ListItem> = app
        ...
        .collect();
    items.push(pseudo_row("+ new session", inner_width, theme));
...
fn render_windows(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) {
    ...
    let mut items: Vec<ListItem> = app
        .windows()
        .iter()
        .map(|w| window_row(w, app, focused, inner_width, theme))
        .collect();
    items.push(pseudo_row("+ new window", inner_width, theme));
...
fn render_panes(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) {
    ...
    let mut items: Vec<ListItem> = app
        .panes()
        .iter()
        .map(|p| pane_row(p, app, focused, inner_width, theme, home.as_deref()))
        .collect();
    items.push(pseudo_row("+ split pane", inner_width, theme));
...
fn session_row(
    ...
    styled_row(vec![line], selected, column_focused, theme)
}
...
fn pane_row(
    ...
    let line1 = Line::from(vec![
        Span::styled(format!("{dot} "), Style::default().fg(dot_color)),
        Span::styled(p.command.clone(), Style::default().fg(theme.fg())),
        Span::raw("  "),
    ...
    let _ = width;
    styled_row(vec![line1, line2], selected, column_focused, theme)
}

fn pseudo_row(label: &str, width: u16, theme: &Theme) -> ListItem<'static> {
...
/// (panes) aligned since every line gets the same prefix width.
fn styled_row(
    ...
    let bg = if selected && column_focused {
        theme.selection_bg()
    } else if selected {
        theme.mantle
    } else {
    ...
    for (i, line) in lines.iter_mut().enumerate() {
        let gutter = if i == 0 && selected && column_focused {
            Span::styled("\u{2502} ", Style::default().fg(theme.active()))
        } else {
            Span::raw("  ")
        };
